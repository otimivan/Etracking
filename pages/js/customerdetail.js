...
$(document).ready(function () {
    ...
    if (GetQueryStringParams('empId') != undefined) {
        ...
        $.ajax(
        {
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (record.length > 0) {
                    ...
                    $("#imgPic").attr('src', 'http://etrackingapp.000webhostapp.com/ourApp/images/employee/' + record[0].photo);
        ...
        $('#page-wrapper').mask("");
        var formURL = 'http://etrackingapp.000webhostapp.com/ourApp/service/admin/getcustomerdetail.php?empId=' + GetQueryStringParams('empId');
        $.ajax(
        {
            url: formURL,
            type: "POST",
    ...
    $('#spChange').click(function () {
        if ($('#spChange').html() == 'Change') {
            ...
            var formURL = 'http://etrackingapp.000webhostapp.com/ourApp/service/admin/employeechangepassword.php';
            $.ajax(
           {
               url: formURL,
               type: "POST",
    ...
    function getTodayAttendance() {
        ...
        var formURL = 'http://etrackingapp.000webhostapp.com/ourApp/service/admin/searchattendance.php';
        $.ajax(
        {
            url: formURL,
            type: "POST",
...
function getImages(imgId) {
    $('#mark').mask("");
    var formURL = 'http://etrackingapp.000webhostapp.com/ourApp/service/admin/getimagedetail.php?imgId=' + imgId;
    $.ajax(
    {
        url: formURL,
        type: "POST",
            ...
            if (record.length > 0) {
                ...
                for (var i = 0; i < record.length; i++) {
                    tbl += '<div class="col-lg-4">'
                            + '        <img src="http://etrackingapp.000webhostapp.com/ourApp/images/product/' + record[i].imgName + '" alt="img" width="150" height="215" />'
                            + '    </div>';
                }
                $('#rowImages').html(tbl);
                $('#mark').unmask();
